//! **Endpoint** — the fast/light/accurate half (engine_endpoint_backend.md §1–§7).
//!
//! Differences from the base `edr_engine::Engine`, all deliberate:
...
use edr_engine::pattern::{Mask, Pattern, RoleSource, Scope, Step, StepMatch};
use edr_engine::rules::{RateState, RuleSet, Tactic};
use edr_engine::{Decision, Verdict, VerdictKind, DEFAULT_RULES};
...
// ---- scoring weights (mirror edr_engine::lib §6) ---------------------------
...
// ---- helpers (ported from edr_engine::lib, using only public API) ----------
