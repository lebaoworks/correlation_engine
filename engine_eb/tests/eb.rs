...
//!
//! Reuses the base crate's shipped datasets (../engine/datasets). Checks that the
...
use edr_engine_eb::{Chain, Decision, Pipeline, VerdictKind};

const DS: &str = "../engine/datasets";
...
fn lsass_dump_blocked_at_read_with_chain() {
    ...
    let mut pipe = Pipeline::from_rules_file("../engine/rules/lsass_dump.rules").expect("rules load");
